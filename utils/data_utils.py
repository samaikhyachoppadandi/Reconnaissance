...
_df = pd.read_csv(data_dir+'/dstl/train_wkt_v4.csv',
...
_df1 = pd.read_csv(data_dir+'/dstl/grid_sizes.csv',
                   names=['ImageId', 'Xmax', 'Ymin'], skiprows=1)

# sample_submission.csv is the file for submission
_df2 = pd.read_csv(data_dir+'/dstl/sample_submission.csv',
                   names=['ImageId', 'ClassId', 'MultipolygonWKT'], skiprows=1)
...
class ImageData():
    ...
    def get_image_path(self):
        ...
        '''
        return {
            '3': '{}/dstl/three_band/{}.tif'.format(data_dir, self.image_id),
            'A': '{}/dstl/sixteen_band/{}_A.tif'.format(data_dir, self.image_id),
            'M': '{}/dstl/sixteen_band/{}_M.tif'.format(data_dir, self.image_id),
            'P': '{}/dstl/sixteen_band/{}_P.tif'.format(data_dir, self.image_id)
        }
    ...
    def image_stack(self):
        ...
        warp_matrix_a = np.load(data_dir+'/utils/image_alignment/{}_warp_matrix_a.npz'.format(self.image_id))
        warp_matrix_m = np.load(data_dir+'/utils/image_alignment/{}_warp_matrix_m.npz'.format(self.image_id))
